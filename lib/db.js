...
function normalizeGoal(goal) {
  ...
  const milestones = Array.isArray(raw.milestones)
    ...
    : Array.isArray(parseJSON(raw.milestones))
      ? parseJSON(raw.milestones).map((m) =>
          typeof m === "object" && m !== null
            ? { title: m.title ?? "", done: Boolean(m.done) }
            : { title: String(m ?? ""), done: false }
        )
...
/**
 * @returns {Array<{date: string, count: number, focusMinutes: number}>} Heatmap data for last 365 days
 */
function getHeatmapData() {
  ...
  return db
    .prepare(
      "SELECT date, tasksCompleted as count, focusMinutes FROM daily_logs WHERE date >= @from ORDER BY date ASC"
    )
    .all({ from: yearAgo });
}
