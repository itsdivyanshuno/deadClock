 ...
 *
 * All functions are async. Uses @libsql/client in production,
 * falls back to better-sqlite3 + .local.db when env vars are absent.
 */
...
async function getClient() {
  ...
  dbPromise = (async () => {
    ...
    return {
      execute(sql)        { return Promise.resolve(local.exec(sql)); },
      batch(stmts)        { return Promise.resolve(stmts.map(s => local.exec(s.statement))); },
      prepare(sql) {
...
// ── Public state API (all async now) ─────────────────────────────────────────

async function loadState() {
  const db = await getClient();
  const tasks = (await db.prepare("SELECT * FROM tasks").all()).map(normalizeTask);
  const goals = (await db.prepare("SELECT * FROM goals").all()).map(normalizeGoal);
  const chatHistory = await db.prepare("SELECT role, content FROM chatHistory ORDER BY id ASC").all();
...
async function saveState(state) {
  ...
  await db.batch([
    ...
    { statement: "DELETE FROM goals" },
    { statement: "DELETE FROM chatHistory" },
  ]);
  ...
  for (const raw of state.tasks) {
    ...
    await insertTask.run(t.id,t.title,t.description,t.priority,t.deadline,t.status,t.category,t.estimatedTime,stringifyJSON(t.subtasks),t.completedAt,t.createdAt);
  }
  ...
  for (const raw of state.goals) {
    ...
    await insertGoal.run(g.id,g.title,g.description,g.deadline,g.progress,stringifyJSON(g.milestones));
  }
  const insertChat = db.prepare("INSERT INTO chatHistory (role, content) VALUES (?, ?)");
  for (const msg of state.chatHistory.slice(-100)) await insertChat.run(msg.role, msg.content);
}
...
async function getCurrentStreak() {
  const row = await (await getClient()).prepare("SELECT currentStreak FROM user_stats WHERE id = 1").get();
  return row?.currentStreak ?? 0;
}
async function getLongestStreak() {
  const row = await (await getClient()).prepare("SELECT longestStreak FROM user_stats WHERE id = 1").get();
  return row?.longestStreak ?? 0;
}
async function getTotalCompletions() {
  const row = await (await getClient()).prepare("SELECT totalCompletions FROM user_stats WHERE id = 1").get();
  return row?.totalCompletions ?? 0;
}
async function getHeatmapData() {
  const today = new Date().toISOString().slice(0,10);
  const yearAgo = new Date(Date.now()-365*864e5).toISOString().slice(0,10);
  return await (await getClient()).prepare(
...
async function getDailyLogs(days=30) {
  const rows = await (await getClient()).prepare(
    "SELECT date, tasksCompleted, focusMinutes FROM daily_logs ORDER BY date DESC LIMIT ?"
  ).all(days);
  return rows.reverse();
}
async function getAchievements() {
  return await (await getClient()).prepare("SELECT * FROM achievements ORDER BY unlockedAt DESC").all();
}
async function unlockAchievement(id, title, description, icon) {
  await (await getClient()).prepare(
    "INSERT OR IGNORE INTO achievements (id,title,description,icon) VALUES (?,?,?,?)"
  ).run(id, title, description, icon);
}
async function checkAchievements() {
  const db = await getClient();
  const stats = await db.prepare("SELECT totalCompletions,currentStreak FROM user_stats WHERE id = 1").get();
  const tasks = await db.prepare("SELECT * FROM tasks").all();
  const goals = await db.prepare("SELECT * FROM goals").all();
  const today = new Date().toISOString().slice(0,10);
  const totalToday = (await db.prepare("SELECT tasksCompleted FROM daily_logs WHERE date = ?").get(today))?.tasksCompleted ?? 0;
  const checks = {
