 ...
 *     to read and mutate the user's task store.
  ...
  switch (name) {
    ...
    case "get_reminders": {
      ...
      const urgent = state.tasks.filter((t) => {
        ...
        switch (args.timeframe) {
          ...
          case "this_week":
            return diffHours > 48 && diffHours <= 168;
          default:
    ...
    case "break_down_goal": {
      ...
      if (goal.milestones.length === 0) {
        return {
          success: false,
          message: 'This goal has no milestones yet. Add milestones first using the "add_goal" tool with a milestones array.',
        };
      }
 ...
 * @returns {number} Hours as a positive integer. Returns 1 for unparseable / zero / negative inputs.
 */
