...
export function CommandPalette({
  ...
  const commands: CommandItem[] = [
    ...
    },
    {
      id: "dashboard",
      label: "Dashboard",
      description: "Productivity overview and insights",
    ...
    },
    {
      id: "analytics",
      label: "Analytics",
      description: "Streaks, trends, and completion stats",
      icon: BarChart3,
      action: () => {
    ...
    },
    {
      id: "heatmap",
      label: "Activity",
      description: "6-week activity heatmap",
      icon: TrendingUp,
      action: () => {
        onNavigate("heatmap");
        onOpenChange(false);
    ...
    },
    {
      id: "reflection",
      label: "Reflect",
      description: "End-of-day reflection journal",
      icon: Moon,
      action: () => {
    ...
    },
    {
      id: "settings",
      label: "Settings",
      description: "App preferences and shortcuts",
      icon: Settings2,
      shortcut: [","],
      action: () => {
        onNavigate("settings");
          ...
          >
            <div className="flex items-center gap-3 px-4 border-b border-border">
              ...
              <input
                ...
                autoFocus
              />
              <kbd className="hidden sm:flex items-center gap-1 text-[10px] font-mono text-text-muted border border-border rounded-md px-1.5 py-0.5">
                esc
              </kbd>
            </div>
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <button
                      key={item.id}
                      onClick={item.action}
                      className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm hover:bg-border-light hover:shadow-sm transition-all duration-150"
                    >
                      ...
                      {item.shortcut && (
                        <div className="flex items-center gap-1">
                          {item.shortcut.map((key, idx) => (
                            <kbd
                              key={`${item.id}-${key}-${idx}`}
                              className="text-[10px] font-mono text-text-muted border border-border rounded-md px-1.5 py-0.5"
                            >
                              {key}
                            </kbd>
                          ))}
            ...
            <div className="border-t border-border px-4 py-2 flex items-center gap-4 text-[10px] text-text-muted">
              <span className="flex items-center gap-1.5">
                <kbd className="font-mono border border-border rounded-md px-1 py-0.5">↑↓</kbd>{" "}
                Navigate
              </span>
              <span className="flex items-center gap-1.5">
                <kbd className="font-mono border border-border rounded-md px-1 py-0.5">↵</kbd>{" "}
              ...
              <span className="flex items-center gap-1.5">
                <kbd className="font-mono border border-border rounded-md px-1 py-0.5">esc</kbd>{" "}
                Close
              </span>
            </div>
          </motion.div>
